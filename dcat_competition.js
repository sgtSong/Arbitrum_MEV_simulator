...
const competitorRpcLogPath = path.join(process.cwd(), "competitor_rpc.log");

function parseCompetitorLogByBlock(logText) {
    const entryByBlock = new Map();

    for (const line of logText.split(/\r?\n/)) {
        let match = line.match(/^\[block (\d+)\] Detected target-pool swap tx: (0x[a-fA-F0-9]+)$/);
        if (match) {
            const blockNumber = Number(match[1]);
            const entry = entryByBlock.get(blockNumber) ?? {};
            entry.detectedTxHash = match[2];
            entryByBlock.set(blockNumber, entry);
            continue;
        }

        match = line.match(/^\[block (\d+)\] Competitor tx submitted: (0x[a-fA-F0-9]+)$/);
        if (!match) continue;

        const blockNumber = Number(match[1]);
        const entry = entryByBlock.get(blockNumber) ?? {};
        entry.competitorTxHash = match[2];
        entryByBlock.set(blockNumber, entry);
    }
...
async function getTxBlockNumber(provider, txHash) {
...
async function main() {
    ...
    let competitorExitPromise = Promise.resolve();
    const stopCompetitor = async () => {
        if (competitorProcess && !competitorProcess.killed) {
            competitorProcess.kill("SIGINT");
        }
        await competitorExitPromise.catch(() => {});
    ...
    if (runCompetitor) {
        ...
        competitorExitPromise = new Promise((resolve) => {
    ...
    for (let round = 0; round < rounds; round++) {
        ...
        const sendswapAmount = 1n * 10n ** 18n; // 30_000n * 10n ** 18n;  //30,000 KITA
        const receiveswapAmount = 1n * 10n ** 18n; // 25_453n * 10n ** 18n; //25,453 ITTA
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
            try {
                ...
                if (placement1.relative >= relativeThreshold && placement1.relative < 1.0) {    //exclude when the threshold is exactly 1.0
                    ...
                    //     const balance = 45_300n * 10n ** 18n; //amount to rebalance back to pool (both sides of the swap)
    ...
    let competitorLogByBlock = new Map();
    if (runCompetitor) {
        await stopCompetitor();

        try {
            const competitorLogText = fs.readFileSync(competitorRpcLogPath, "utf8");
            competitorLogByBlock = parseCompetitorLogByBlock(competitorLogText);
        } catch (err) {
            console.error(`Failed to read competitor log ${competitorRpcLogPath}: ${err.message || err}`);
        }
    }
    ...
    for (let index = 0; index < results.length; index++) {
        ...
        if (!runCompetitor || !incident) continue;

        const senderBlockNumber = incident.sender.block_number;
        const senderTxHash = incident.sender.tx_hash;
        const receiverBlockNumber = incident.receiver.block_number;
        const logEntry = senderBlockNumber !== null ? competitorLogByBlock.get(senderBlockNumber) ?? null : null;

        if (
            !logEntry ||
            !logEntry.detectedTxHash ||
            !logEntry.competitorTxHash ||
            !senderTxHash ||
            logEntry.detectedTxHash.toLowerCase() !== senderTxHash.toLowerCase() ||
            receiverBlockNumber === null
        ) {
            incident.win_competitor = null;
            r.win_competitor = null;
            continue;
        }

        try {
            const competitorBlockNumber = await getTxBlockNumber(provider, logEntry.competitorTxHash);
            const winCompetitor = competitorBlockNumber !== null ? receiverBlockNumber < competitorBlockNumber : null;
            incident.win_competitor = winCompetitor;
            r.win_competitor = winCompetitor;
        } catch (err) {
            console.error(`Failed to resolve competitor tx ${logEntry.competitorTxHash}: ${err.message || err}`);
            incident.win_competitor = null;
            r.win_competitor = null;
        }
    }
